...
def attention(query, key, value, mask=None, dropout=None):
    ...
    :param query: [batch_size, h, sequence_len, d_model]
    :param key: [batch_size, h, sequence_len, d_model]
    :param value: [batch_size, h, sequence_len, d_model]
    """
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1) / math.sqrt(d_k))
    # 多维矩阵相乘需保证最后两维匹配
    # (batch_size, h, sequence_len, d_model) * (batch_size, h, d_model, sequence_len)
    ...
    #  res--> (batch_size, h, sequence_len, embed_dim)
